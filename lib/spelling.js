 ...
 * @returns {Promise<Array<{word: string, line: number, column: number, suggestions: string[]}>>}
 */
 ...
 * @returns {Promise<Array<{word: string, line: number, column: number, suggestions: string[], file: string}>>}
 */
export async function checkFile(filePath, options = {}) {
  const text = fs.readFileSync(filePath, 'utf-8');
  const issues = await checkSpelling(text, options);
